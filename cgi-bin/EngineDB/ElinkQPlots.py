...
tempEC = pd.read_csv('./static/files/ElinkQuality.csv')
EC = tempEC.dropna()
...
def EClockGaussian(sel_phase):
    ...
    while min_date <= today:
        date_range.append(min_date)
        min_date += datetime.timedelta(days=1)
    columns = ['Type ID', 'Person Name', 'Outcome', 'Start Date', 'End Date']
    data = [ds.data['Type ID'].tolist(), ds.data['Person Name'].tolist(), ds.data['Outcome'], date_range, date_range]
