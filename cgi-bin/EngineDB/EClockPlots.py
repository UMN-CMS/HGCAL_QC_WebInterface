...
tempEC = pd.read_csv('./static/files/EClockRates.csv')
tempEC = tempEC.set_index('Test ID')
EC = tempEC.stack()
EC = EC.reset_index()
EC = EC.rename(columns={0:'Rate'})
EC = EC.dropna()
...
def Histogram(columns, data, view, widgets, modules, slider):
    ...
    dt = ColumnDataSource(data={'Type ID':[], 'Full ID':[], 'Person Name':[], 'Time':[], 'Outcome':[], 'level_1':[], 'Rate':[]})
...
def Filter():
    ...
    table_columns = [
                    ...
                    TableColumn(field='level_1', title='Clock'),
...
for (let k = 0; k < modules.length; k++) {
    const indices = view.filters[0].compute_indices(data);
    let mask = new Array(data.data['Rate'].length).fill(false);
    [...indices].forEach((x)=>{mask[x] = true;})

    for (let j = 0; j < data.get_length(); j++) {
        if (mask[j] == true && data.data['level_1'][j] == modules[k]){
            mask[j] = true;
        } else {
            mask[j] = false;
        }
    }
    const good_data = data.data['Rate'].filter((_,y)=>mask[y])
    let mean = d3.mean(good_data)
    let std = d3.deviation(good_data)
    for (let sn = 0; sn < serial_numbers.length; sn++) {
        const indices = view.filters[0].compute_indices(data);
        let mask_sn = new Array(data.data['Rate'].length).fill(false);
        [...indices].forEach((x)=>{mask_sn[x] = true;})
        for (let j = 0; j < data.get_length(); j++) {
            if (mask_sn[j] == true && data.data['level_1'][j] == modules[k] && data.data['Full ID'][j] == serial_numbers[sn]){
                mask_sn[j] = true;
                let x = data.data['Rate'][j] - mean;
                if (std == 0) {
                    let chi = 0;
                    residules.push(chi)
                } else {
        ...
        const good_data_sn = data.data['Rate'].filter((_,y)=>mask_sn[y])
