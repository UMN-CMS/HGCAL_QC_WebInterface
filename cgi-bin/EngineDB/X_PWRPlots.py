...
def Histogram(columns, data, view, widgets):
    ...
    dt = ColumnDataSource(data={'Sub Type':[], 'Full ID':[], 'Person Name':[], 'Time':[], 'Outcome':[], 'Voltage':[]})
...
'''))
    ...
    plot_json = json.dumps(json_item(row(column(row(w[0:3]), row(w[3:5]), row(w[5:]), p, data_table, data_table_2), layout)))
    return plot_json
