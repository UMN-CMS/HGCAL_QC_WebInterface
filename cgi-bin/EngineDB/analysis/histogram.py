...
def main():
    ...
    text_input = TextAreaInput(
        value='map(select( (.type_id | test("EL10[EW]1")) and .test_name == "Eye Opening")) | .[] | .attach.test_data.DAQ.area',
        title="Query:",
    )
    ...
    callback = CustomJS(
        ...
        const d = fetch(base_url + search_params.toString()).then(
           x=> {
           x.json().then(d =>{ console.log(d); source.data = d;}); })
        """,
    )
    ...
    ylabel_input.js_on_change(
        "change", CustomJS(args=dict(source=cds, p=p.yaxis), code ='''
        console.log(cb_obj.value);
        p[0].axis_label=cb_obj.value; ''')
    )
