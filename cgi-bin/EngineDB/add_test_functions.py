...
def get_is_ready_for_thermal(full_id):
...
def get_thermal_failures(full_id):
    db = connect(0)
    cur = db.cursor()
    ...
    cur.execute('''
        ...
        JOIN Test_Type TT 
        ON T.test_type_id = TT.test_type
        WHERE B.full_id="%s"
        AND TT.name = "Thermal Cycle"
        AND T.successful = 0
    ''' % full_id)
...
def set_daq_chip_id(sn, test_id):
    ...
    if sn[3:5] == 'EL':
        daq_chip_id = attach['DAQ']['id']
    if sn[3:5] == 'EH':
        daq_chip_id = attach['DAQ1']['id']
...
def add_test(person_id, test_type, serial_num, success, comments, config_id):
    ...
    cur.execute('select type_id from Board_type where type_sn="%s"' % serial_num[3:9])
    type_id = cur.fetchall()[0][0]
