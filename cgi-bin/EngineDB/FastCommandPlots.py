...
tempEC = pd.read_csv(mTD.get_fast_command())
EC = tempEC.dropna()
...
def Histogram(data, view, widgets, channels, phases):
    ...
    dt = ColumnDataSource(data={'Sub Type':[], 'Full ID':[], 'Person Name':[], 'Time':[], 'Outcome':[], 'Channel':[], 'Phase':[], 'Bit Errors':[]})
...
def Gaussian2(data, view, widgets, serial_numbers, modules, n_sigma, phases):
    ...
    for i in modules:
        ...
        td[i] = ColumnDataSource(data={'Serial Number':[], 'Phase':[], 'Deviation':[], 'Pass/Fail':[]})
    x = CustomJS(args=dict(hist=hist, data=data, view=view, serial_numbers=serial_numbers, modules=modules, n_sigma=n_sigma, pf=pf, td=td, phases=phases),code='''
for (let k = 0; k < modules.length; k++) {
    ...
    for (let p = 0; p < phases.length; p++) {
        ...
        for (let j = 0; j < data.get_length(); j++) {
            if (mask[j] == true && data.data['Channel'][j] == modules[k] && data.data['Phase'][j] == phases[p]){
                mask[j] = true;
            } else {
                mask[j] = false;
            }
        ...
        for (let sn = 0; sn < serial_numbers.length; sn++) {
            ...
            for (let j = 0; j < data.get_length(); j++) {
                if (mask_sn[j] == true && data.data['Channel'][j] == modules[k] && data.data['Full ID'][j] == serial_numbers[sn] && data.data['Phase'][j] == phases[p]){
                    mask_sn[j] = true;
    ...
    hist[modules[k]].data['right'] = right;
    hist[modules[k]].data['left'] = left;
    hist[modules[k]].data['bottom'] = bottom;
    hist[modules[k]].data['top'] = top;
    hist[modules[k]].change.emit()
    ...
    pf[modules[k]].data['pass'] = [0, t_pass];
    pf[modules[k]].data['fail'] = [t_fail, 0];
    pf[modules[k]].change.emit()

    td[modules[k]].data['Serial Number'] = sn_td;
    td[modules[k]].data['Phase'] = mod_td;
    td[modules[k]].data['Deviation'] = chi_td;
    td[modules[k]].data['Pass/Fail'] = pf_td;
    td[modules[k]].change.emit()
}
    ''')
    for widget in widgets:
        widget.js_on_change('value', x)
...
def Gaussian():
    ...
    data = [ds.data['Major Type'].tolist(), ds.data['Sub Type'].tolist(), ds.data['Full ID'], ds.data['Outcome'], date_range, date_range]
