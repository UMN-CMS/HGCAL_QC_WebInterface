...
if form.getvalue('type_id'):
    ...
    cur.execute('select type_sn, name from Board_type where type_sn like "EL%" order by type_sn')
    ld_subtypes = cur.fetchall()

    cur.execute('select type_sn, name from Board_type where type_sn like "EH%" order by type_sn')
    hd_subtypes = cur.fetchall()

    m = max([len(ld_subtypes), len(hd_subtypes)])
    ...
    for s in range(m):
        ...
        print('<td>')
        try:
            print('<a href="board_images.py?type_id=%(id)s">%(id)s</a>' %{'id':ld_subtypes[s][0], 'name': ld_subtypes[s][1]})
        except:
            pass
        print('</td>')
        print('<td>')
        try:
            print('<a href="board_images.py?type_id=%(id)s">%(id)s</a>' %{'id':hd_subtypes[s][0], 'name': hd_subtypes[s][1]})
        except:
            pass
        print('</td>')
        print('</tr>')
