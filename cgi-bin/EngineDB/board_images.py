...
for s in subtypes:
    ...
    for sn in serial_numbers[s]:
        ...
        try:
            cur.execute('select image_name from Board_images where board_id=%s and view="Top"' % board_id)
            name_top = cur.fetchall()[0][0]
            cur.execute('select image_name from Board_images where board_id=%s and view="Bottom"' % board_id)
