...
tempup = pd.read_csv(mTD.get_uplink_quality())
up = tempup.dropna()
...
def Histogram(data, view, widgets, modules):
    ...
    dt = ColumnDataSource(data={'Sub Type':[], 'Full ID':[], 'Person Name':[], 'Time':[], 'Outcome':[], 'E Link':[], 'Bit Errors':[]})
    ...
    x = CustomJS(args=dict(hist=hist_data, data=data, view=view, modules=modules, dt=dt),code='''
const type_ids=[];
const full_ids=[];
const people=[];
const times=[];
...
for (let i = 0; i < modules.length; i++) {
    ...
    for (let j = 0; j < data.get_length(); j++) {
        if (data.data['Module'][j] == modules[i] && mask[j] == true){
            mask[j] = true;
            ...
            pathways.push(data.data['Module'][j])
...
def Filter():
    ...
    df_temp = AllData.merge(up, on='Test ID', how='left')
    ...
    modules = np.unique(ds.data['Module']).tolist()
    ...
    hds, dt = Histogram(ds, view, widgets.values(), modules)
    # creates the figure object
    p = figure(
        title='Uplink Quality',
...
for (let k = 0; k < modules.length; k++) {
    ...
    for (let j = 0; j < data.get_length(); j++) {
        if (mask[j] == true && data.data['Module'][j] == modules[k]){
            mask[j] = true;
        } else {
            mask[j] = false;
        }
    ...
    for (let sn = 0; sn < serial_numbers.length; sn++) {
        ...
        for (let j = 0; j < data.get_length(); j++) {
            if (mask_sn[j] == true && data.data['Module'][j] == modules[k] && data.data['Full ID'][j] == serial_numbers[sn]){
                mask_sn[j] = true;
...
'''))
    ...
    table_columns = [
                    ...
                    TableColumn(field='Module', title='Channel Number'),
