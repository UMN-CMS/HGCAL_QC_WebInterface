...
def decode_label(label):
    ...
    cur.execute('select name from Major_Type where major_code="%s"' % major)
    major_name = cur.fetchall()[0][0]

    cur.execute('select name from Sub_Type where sub_code="%s"' % sub)
    sub_name = cur.fetchall()[0][0]

    return [major, major_name, sub, sub_name, type_code, sn]
