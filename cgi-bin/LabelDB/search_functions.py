...
LabelData = pd.read_csv('./static/files/Labels.csv', parse_dates=['Time'])
MajorTypeData = pd.read_csv('./static/files/MajorType.csv')
SubTypeData = pd.read_csv('./static/files/SubType.csv')
mergetemp = LabelData.merge(MajorTypeData, on='Major Type ID', how='left')
AllData = mergetemp.merge(SubTypeData, on='Sub Type ID', how='left')
AllData.dropna()
...
def makeTable(ds, widgets, view):
    td = ColumnDataSource({'Major Type':[], 'Sub Type':[], 'Type Code':[], 'Full Label':[], 'Date':[], 'Raw Time':[]})
...
let mask = new Array(data.data['Full Label'].length).fill(false);
[...indices].forEach((x)=>{mask[x] = true;})
for (let j = 0; j < data.get_length(); j++) {
    if (mask[j] == true){
        major_types.push(data.data['Major Type'][j])
        sub_types.push(data.data['Sub Type'][j])
        type_codes.push(data.data['Type Code'][j])
        full_labels.push(data.data['Full Label'][j])
...
td.data['Major Type'] = major_types;
td.data['Sub Type'] = sub_types;
td.data['Type Code'] = type_codes;
td.data['Full Label'] = full_labels;
td.data['Date'] = times;
...
def Filter():
    ...
    columns = ['Major Type', 'Sub Type', 'Start Date', 'End Date']
    data = [ds.data['Major Type'].tolist(), ds.data['Sub Type'].tolist(), date_range, date_range]
...
'''
    ...
    table_columns = [
                    TableColumn(field='Major Type', title='Major Type', formatter=bigger_font),
                    TableColumn(field='Sub Type', title='Sub Type', formatter=bigger_font),
                    TableColumn(field='Type Code', title='Type Code', formatter=bigger_font),
                    TableColumn(field='Full Label', title='Full Label', formatter=bigger_font),
                    TableColumn(field='Date', title='Time', formatter=bigger_font),
                    TableColumn(field='Raw Time', title='Raw Time', formatter=bigger_font),
                    ]

    data_table = DataTable(source=td, columns=table_columns, row_height = 40, autosize_mode='fit_columns', height=600, width=1200)

    w = [*widgets.values()]
    major_types = np.unique(ds.data['Major Type'].tolist()).tolist()
    sub_types = {}
    for m in major_types:
        sub_types[m] = np.unique(AllData.query('`Major Type` == @m')['Sub Type'].values.tolist()).tolist()
    update_options = CustomJS(args=dict(sub_types=sub_types, widget=w[1]), code=('''
widget.options = sub_types[this.value]
'''))
    w[0].js_on_change('value', update_options)

    return json.dumps(json_item(row(column(row(w[0:2]), row(w[2:]), data_table))))
