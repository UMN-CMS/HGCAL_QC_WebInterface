...
from .serial_schema import FieldValue


@dataclass
class UnpackedBarcode:
    """
    ...
    :type field_values: Dict[str, :py:class:`.FieldValue` ]
    """
    ...
    def __post_init__(self):
        self.field_values = {k: FieldValue(v) for k, v in self.field_values.items()}


def decode(barcode: str) -> UnpackedBarcode:
    """Decode a barcode.
    ...
    :rtype: :py:class:`.UnpackedBarcode`
    """

    if len(barcode) != 15:
        raise InvalidBarcode(f'Barcode "{barcode}" has length {len(barcode)} != 15.')
    if not barcode.isalnum():
        raise InvalidBarcode(
            f'Barcode "{barcode}" contains non alphanumeric characters.'
        )
    if barcode[:3] != "320":
        raise InvalidBarcode(f'Barcode "{barcode}" starts with {barcode[:3]} != "320"')
    ...
    return UnpackedBarcode(major_type_code, subtype_code, serial_schema.decode(serial))


def encode(barcode_spec: UnpackedBarcode) -> str:
    """Decode a barcode.


    :param barcode_spec: Values to encode
    :type barcode_spec: :py:class:`.UnpackedBarcode`
