...
injection_tile_fields = SerialSchema(
    NumericField("InjectionNumber", "Injection-molding batch number from which the tile was moldeld", 4), 
    MappingField("ReelMagID", "Magazine or Reel Id in which the tile was produced", 1, RDict),
    NumericField("SerialNumber", "Serial number", 3)
)
...
sipm_fields = SerialSchema(
    NumericField("InjectionNumber", "Injection-molding batch number from which the SiPM was moldeld", 4), 
...
def machineTileGetSubtype(major_type, code):
    ...
    if major_type == "BareCast" or major_type == "WrappedCast":
...
def sipmGetSubtype(major_type, code):
    ...
    if code == "04":
        name = "4 mm\ :sup:`2` "
        long_name = major_type.name + "of size 4 mm\ :sup:`2` "
    else:
        name = "9 mm\ :sup:`2` "
        long_name = major_type.name + "of size 9 mm\ :sup:`2` "
    nc = code
