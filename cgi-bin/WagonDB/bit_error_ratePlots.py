...
'''))
    ...
    for major in np.unique(ds_mp.data['Major Type'].tolist()).tolist():
        subtypes[major] = np.unique(mp_temp.query('`Major Type` == @major')['Sub Type'].values.tolist()).tolist()
    serial_numbers = {}
    ...
    all_subtypes = np.unique(ds_mp.data['Sub Type'].tolist()).tolist()
    all_serials = np.unique(ds_mp.data['Full ID'].tolist()).tolist()
