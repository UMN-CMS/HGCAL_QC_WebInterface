...
def makePlot(Test, Data, Board, SN, BitError, Tester, Outcome, StartDate, EndDate):
    ...
    if Test is 'Total':
        fig, ax = plt.subplots()
        if Tester is None or len(Tester) == 1:
            if Outcome[0] == True:
                first = StartDate
            ...
            for t in Tester:
                person_id = PeopleData.query('`Person Name` == @t')['Person ID'].values.tolist()
                if Outcome[0] == True:
                    first = StartDate
                    dates = TestData.query('`Test ID` == @total_tests & `Person ID` == @person_id')['Time']
                    time_series_data = []
                    i = 0
                    while (first <= EndDate):
                        for d in dates:
                        ...
                        first += datetime.timedelta(days=1)
                    x1,y1 = zip(*time_series_data)
                    plt.plot(x1, y1, label = 'Total-' + t)
                if Outcome[1] == True:
                    first = StartDate
                    successful_dates = TestData.query('`Test ID` == @successful_tests & `Person ID` == @person_id')['Time']
                    time_series_data_successful = []
                    j = 0
                    while (first <= EndDate):
                        for d in successful_dates:
                        ...
                        first += datetime.timedelta(days=1)
                    x2,y2 = zip(*time_series_data_successful)
                    plt.plot(x2, y2, label = 'Successful-' + t)
                if Outcome[2] == True:
                    first = StartDate
                    unsuccessful_dates = TestData.query('`Test ID` == @unsuccessful_tests & `Person ID` == @person_id')['Time']  
                    time_series_data_unsuccessful = []
                    k = 0
                    while (first <= EndDate):
                        for d in unsuccessful_dates:
                        ...
                        first += datetime.timedelta(days=1)
                    x3,y3 = zip(*time_series_data_unsuccessful)
                    plt.plot(x3, y3, label = 'Unsuccessful-' + t)
...
makePlot('Total', [True,False,False,False,False,False,True,False,False,False,False,False], None, None, [True,True,False,True,False,False,False,False,False,False,False], ['Bryan', 'Jocelyn', 'Garrett'], [True, True, True], None, None)
