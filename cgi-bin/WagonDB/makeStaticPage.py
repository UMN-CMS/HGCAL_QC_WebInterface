...
# gets directories and makes them if they don't exist
...
# makes the normal pages
pages = publish_config.get_pages()
for p in pages:
    # sends std out to the file, then all print statements are sent there
    sys.stdout = open('%(path)s/../../static_html/%(db)s/%(script)s.html' % {'script': p[:-3], 'db': db_name, 'path': local_path}, 'w')
    # import and run the script, cuts the .py
    script = __import__(p[:-3])
    script.run(True) 

# does the same thing for the pages that need to be made for all of a certain entry
