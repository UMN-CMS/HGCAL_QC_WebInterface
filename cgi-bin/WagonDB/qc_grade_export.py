...
"""
Export WagonDB QC grading information into the CSV + YAML pair consumed by
...
Full usage (all arguments):
    python3 qc_grade_export.py \\
        --user "Jeremy Mans" \\
        --run-name "UMN_wagons_batch3" \\
        --output wagon_grades.csv \\
        --runinfo wagon_qcrun.yaml \\
...
def typecode_from_type_id(type_id):
    """'WE10A1' -> 'WE-10A1' (matches the LABEL_TYPECODE used by the
...
def run(args):
    ...
    try:
        ...
        logger.info("Found %d graded wagon(s)", len(rows))
        if not rows:
...
def run_cli():
    parser = argparse.ArgumentParser(
        description="Export WagonDB QC grades to the CSV + YAML consumed by csv_to_xml.py --task qc_grade."
    )
    ...
    parser.add_argument("--run-name", default=None,
                        help="Unique RUN_NAME (default: wagon_qc_grade_<today>)")
    ...
    if args.run_name is None:
        ...
        args.run_name = f"wagon_qc_grade_{today}"
